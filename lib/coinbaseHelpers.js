...
if (Meteor.isServer) {
    ...
    Meteor.methods({
        ...
        //So, you can send the two amounts to the winner
        sendBTCToWinner: function(){
            cl('In sendBTCToWinner() method.');

            var currentGame = getCurrentGame();
            if (currentGame === null){
                cl('getCurrentGame() returns null');
                throw new Meteor.Error('null', 'getCurrentGame() returns null');
            }

            if (currentGame.status != 'completed'){
                cl('current Game is not completed. Should never happen');
                throw new Meteor.Error('gameNotCompleted', 'current Game is not completed. Should never happen');
            }
            ...
            client.getAccount(coinbasePressItAccount, Meteor.bindEnvironment( function (err, account) {
                ...
                if (currentGame.winner === 'tie'){
                    cl('Game was tied. Sending ' + currentGame.btcAmount + ' btc to both ' +
                        currentGame.player1BTCAddress + ' and ' + currentGame.player2BTCAddress + '.');
                    //account.sendMoney({'to': currentGame.player1BTCAddress,
                    //        'amount': currentGame.btcAmount,
                    //        'currency': 'BTC',
                    //        'idem': currentGame.player1BTCAddress}, function(err, tx) {
                    //        console.log(tx);
                    //    });
                    //account.sendMoney({'to': currentGame.player2BTCAddress,
                    //    'amount': currentGame.btcAmount,
                    //    'currency': 'BTC',
                    //    'idem': currentGame.player2BTCAddress}, function(err, tx) {
                    //    console.log(tx);
                    //});
                }
                else if (currentGame.winner === 'player1'){
                    cl('Game was won by Player 1. Sending ' + 2*currentGame.btcAmount + ' to ' +
                    ...
                    cl('Game was won by Player 2. Sending ' + 2*currentGame.btcAmount + ' to ' +
                    ...
                    cl('Game must be in tie, player1 or player2 winner state but is' + currentGame.winner);
